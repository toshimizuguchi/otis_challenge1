    ...
    <div className="space-y-4 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className={`lg:col-span-7 bg-slate-900/90 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-sm space-y-4 ${
          ...
          {/* Estado de espera quando não há chamado corretivo selecionado */}
          {selectedItemType === 'CALL' && !activeCall && (
            <div className="p-8 sm:p-12 text-center bg-slate-950/60 rounded-2xl border border-dashed border-slate-800 space-y-4">
              <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 flex items-center justify-center mx-auto shadow-sm">
              ...
              <div className="space-y-1 max-w-md mx-auto">
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                  Viatura em Prontidão
                </span>
                <h3 className="text-base font-bold text-white">Nenhum chamado corretivo atribuído</h3>
                <p className="text-xs text-slate-400 leading-relaxed">
                  Você está sem ocorrências emergenciais no momento. Assim que um chamado for aberto e despachado pela IA para o seu perfil, ele aparecerá aqui com rota GPS e diagnóstico prévio.
                </p>
              </div>
              <div className="flex items-center justify-center gap-2 pt-2">
                <button
                  onClick={() => setSelectedItemType('MAINTENANCE')}
                  className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 text-xs font-bold transition-all shadow cursor-pointer"
                >
                  Ver Tarefas Preventivas ({maintTasks.length})
                </button>
              </div>
            </div>
          )}
